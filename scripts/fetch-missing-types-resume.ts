// scripts/fetch-missing-types-resume.ts
// グレードがない車種のみの型式を取得（186番から再開）
...
async function main() {
  console.log(`\n=== グレードなし車種の型式取得開始（${SKIP_COUNT}番からスキップ）===\n`);
  ...
  console.log(`${models.length}車種中、${SKIP_COUNT}番以降を処理します\n`);
  ...
  try {
    for (let i = SKIP_COUNT; i < models.length; i++) {
      ...
      try {
        ...
        await page.goto(typeUrl, { waitUntil: 'networkidle', timeout: 20000 });
        await sleep(1000);
